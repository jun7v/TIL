...
```
라이브는 원본 데이터가 변경되면 같이 바뀌고, 추출은 원본 데이터를 따로 복사해놓고 사용
엑셀로 작업하는 우리는 크게 상관 없지만, 데이터베이스 연결해서 쓸 때는 중요할 듯
```
...
```
관계(릴레이션)을 태블로에서는 권장(원본 독립성)
의도적으로 필터링해야 하거나 복제할 때는 조인
```
...
```
'반품된 주문만' 필요하면 겹치는 데이터를 찾기 위해 inner join
3027개 행
```
## 6강. 데이터결합 혼합
...
```
혼합은 실제로 데이터를 합치는 것이 아니라 따로 계산하고 합쳐서 나타내는 것
```
...
```
릴레이션이랑 조인은 이런 데이터를 합치는게 아니라 칼럼을 늘리는데 적합함
밑으로 row를 붙일 때는 유니온
CARD_SUBWAY_MONTH_2024* 하면 될 것 같아요
```
...
```
오프라인 사용시에는 추출이 적합.
그런데 보통 이런 경우에는 데이터를 태블로로 추출하는게 아니라 원본채로 다운로드 해두는게 맞지 않을까요??
```
