...
```
차원: 정성적인 값, 개별적으로 구분됨, 집계 안됨
측정값: 정량적인 값, 집계 가능, 연산 가능
```
...
```
지역-profit/sales-연도의 계층으로 보기 위해 측정값 선반으로 묶어둔 것
개별 칼럼을 행에 이중축으로 올리면 그쪽이 선반영
시각화 계층 설정 시 필요
```
...
```
1. 필터: 국가/지역 필터링 -> South Korea, China, Japan
2. 매출과 수익 행으로 -> 이중축
3. 국가/지역이랑 주문 날짜 열로
4. 주문 날짜를 월(2015년 5월) & 연속형으로 변경 -> 이래야 연속 시계열 로 볼 수 있음
5. 수익이랑 매출이랑 범위가 다르니까 수익 축을 축 동기화
```
